...
export function WhatsappSettingsPage({ user }: { user: UserContext }) {
  ...
  const reconnecting = status.data.status === 'reconnecting';
  const mutationError = connect.error ?? disconnect.error;
  return (
    <div className="page page--settings">
      <header className="settings-page-header"><div><span className="eyebrow">Channel komunikasi</span><h1>Koneksi WhatsApp</h1><p>Hubungkan nomor WhatsApp brand untuk menerima dan membalas percakapan.</p></div><span className={`connection-chip ${connected ? 'connection-chip--connected' : ''}`}><span className="status-dot" />{connected ? 'Terhubung' : 'Perlu perhatian'}</span></header>
      <div className="settings-grid">
        <section className="panel connection-card">
          ...
          <footer>{connected ? <><button className="button button--secondary" disabled={connect.isPending || disconnect.isPending} onClick={() => connect.mutate()}><RefreshCw size={17} />Hubungkan ulang</button><button className="button button--danger-ghost" disabled={disconnect.isPending || connect.isPending} onClick={() => { if (window.confirm('Keluar dari WhatsApp dan hapus sesi perangkat untuk brand ini?')) disconnect.mutate(); }}><LogOut size={17} />Keluar & hapus sesi</button></> : <button className="button button--primary" disabled={connect.isPending} onClick={() => connect.mutate()}>{reconnecting ? <RefreshCw size={17} /> : <QrCode size={17} />}{reconnecting ? 'Coba lagi sekarang' : 'Tampilkan QR'}</button>}</footer>
        </section>
